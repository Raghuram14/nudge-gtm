...
export function generateStaticParams(): Array<Params> {
  return loadArticles("learn").map((article) => ({ slug: article.frontmatter.slug }));
}
...
export async function generateMetadata({
  ...
  const article = loadArticleBySlug("learn", slug);
  if (!article) {
    return {};
  }
  const path = article.frontmatter.canonical ?? `/learn/${slug}`;
